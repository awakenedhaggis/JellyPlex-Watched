...
class JellyfinEmby:
    ...
    def query(self, query, query_type, identifiers=None):
        try:
            ...
            logger(f"{self.server_type}: Query {query_type} {query}\nResults {results}\n{e}", 2)
            raise Exception(e)
    ...
    def get_user_library_watched(
        ...
        try:
            ...
            logger(
                f"{self.server_type}: Got watched for {user_name} in library {library_title}", 1
            )
            if library_title in user_watched[user_name]:
                logger(f"{self.server_type}: {user_watched[user_name][library_title]}", 3)
    ...
    def update_user_watched(
        ...
        try:
            logger(
                f"{self.server_type}: Updating watched for {user_name} in library {library}", 1
            )
            (
                videos_shows_ids,
                videos_episodes_ids,
